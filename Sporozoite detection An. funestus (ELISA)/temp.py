...
#%%
df = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\sporozoite_full.csv")
df.head()
...
# X_train, X_val, y_train, y_val = train_test_split(X, y, test_size= .1, random_state = 42, shuffle = True, stratify = y)
...
# model = BalancedBaggingClassifier(n_estimators = 500, sampler = (NearMiss(version=1)))

# scores = cross_val_score(model, X_new, y_train, scoring = 'accuracy', cv = kf)
# print('Mean ROC AUC: %.2f' % np.mean(scores))
...
# # grid_result = gsc.fit(X_new, y_train)

# # # print best parameter settings
# # print("Best: %.2f using %s" % (grid_result.best_score_,
...
# y_val_pred = model.predict(trans_X_val)
...
X_train, X_val, y_train, y_val = train_test_split(X_res, y_res, test_size= .1, random_state = 42, shuffle = True)
print('The shape of X train index : {}'.format(X_train.shape))
print('The shape of y train index : {}'.format(y_train.shape))
print('The shape of X val index : {}'.format(X_val.shape))
print('The shape of y val index : {}'.format(y_val.shape))
...
models.append(("LR", LogisticRegressionCV(multi_class = 'ovr', cv=kf, random_state=SEED, max_iter=2000)))
...
sns.boxplot(x=results_df['Model'], y=results_df['Accuracy'])
sns.despine(offset=10, trim=True)
# plt.title("Algorithm comparison", weight="bold")
plt.xticks(rotation=90)
plt.yticks(np.arange(0.2, 1.0 + .05, step = 0.1))
plt.ylabel('Accuracy', weight = 'bold');
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\Algorithm selection_elisa.png"), dpi = 500, bbox_inches="tight")
...
with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\classifier.pkl', 'wb') as fid:
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\_averaged_CM_full_wn_elisa.png"), dpi = 500, bbox_inches="tight")

#%%
# Results
kf_results.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_kfCV_record.csv", index=False)
kf_results = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_kfCV_record.csv")

# Accuracy distribution
crf_acc_distrib = kf_results["Accuracy"]
crf_acc_distrib.columns=["Accuracy"]
crf_acc_distrib.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_acc_distrib.csv", header=True, index=False)
crf_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_acc_distrib.csv")
...
crf_per_class_acc_distrib.dropna().to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_per_class_acc_distrib.csv")
crf_per_class_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_per_class_acc_distrib.csv", index_col=0)
...
crf_per_class_acc_distrib_describe.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_per_class_acc_distrib.csv")
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\per_class_acc_distrib_full_wn.png"), dpi = 500, bbox_inches="tight")
...
rskf_results = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\crf_kfCV_record.csv")
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\_feature_impces_full_wn.png"), dpi = 500, bbox_inches="tight")
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\CM_full_wn_val_elisa.png"), dpi = 500, bbox_inches="tight")
...
cr_full_wn_val.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\classification_report_full_wn_val_elisa.csv")
...
with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\important_wavenumbers.txt', 'w') as outfile:
...
# load age data
df_age = pd.read_csv("E:\Sporozoite\wild_funestus_age.dat", delimiter = '\t')
...
new_val_df = pd.concat([y_test, X_test], axis = 1)
...
new_data_age_df = pd.concat([new_val_df, df_age], axis = 0, join = 'outer')
...
plot_confusion_matrix(cm_val_2, text=True, normalise=True, classes = class_names)
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\CM_age_sporozoite_elisa.png"), dpi = 500, bbox_inches="tight")
...
cr_full_wn_val_age = cr_full_wn_val_age.iloc[1:]
cr_full_wn_val_age.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\classification_report_full_wn_val_age.csv")
...
full_data_df = pd.read_csv("E:\Sporozoite\Biological_attr.dat", delimiter= '\t')
full_data_df.head()
...
pcr_data_df = pd.read_csv("E:\Sporozoite\PCR data-35cycles-Cy5-FAM.csv")
pcr_data_df.head()
...
plot_confusion_matrix(cm_val_3, text=True, normalise=True, classes = class_names)
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\CM_full_wn_val_pcr.png"), dpi = 500, bbox_inches="tight")
...
cr_full_wn_val_pcr = cr_full_wn_val_pcr.iloc[1:]
cr_full_wn_val_pcr.to_csv("E:\Sporozoite\Results\classification_report_full_wn_val_pcr.csv")



#%%

# with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\Phd data\Results\StandardML - Allwn - PCR + ELISA data 35cycle + FAM + CY5\important_wavenumbers.txt') as json_file:
#     important_wavenumb_2 = json.load(json_file)


# #%%
# important_wavenumb_combined = important_wavenumb + important_wavenumb_2
# important_wavenumb_combined
...
# result = checkIfDuplicates(important_wavenumb_combined)
# if result:
...
# print([item for item, count in Counter(important_wavenumb_combined).items() if count > 1])

# common_wavenumbers_elisa_PCR = [item for item, count in Counter(important_wavenumb_combined).items() if count > 1]

# # store these duplicates in text files (they are common)
# with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\common_wavenumbers_elisa_PCR.txt', 'w') as outfile:
#      json.dump(common_wavenumbers_elisa_PCR, outfile)

# #%%

# # remove duplicates

# important_wavenumb_combined_2 = list(set(important_wavenumb_combined))

# result_2 = checkIfDuplicates(important_wavenumb_combined_2)
...
# with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\common_wavenumbers_elisa_PCR.txt') as json_file:
#     important_wavenumb_2 = json.load(json_file)

# # Select the important wavenumbers from the main dataframe
# new_data = features[[wavenumber for wavenumber in features.columns if wavenumber in important_wavenumb_2]]
# new_data


# #%%
# # Fit the model using data with important wavenumbers only
# # rescalling the data (undersampling the over respresented class - negative class)

# rus = NearMiss(version = 1, n_neighbors = 3)
# X_res_new, y_res_new = rus.fit_resample(new_data, y)
# y_res_count = collections.Counter(y_res_new)
# print(y_res_count)

# # Standardise inputs using standard scaler

# X_train_new, X_val_new, y_train_new, y_val_new = train_test_split(X_res_new, y_res_new, test_size= .1, random_state = 42, shuffle = True)
# print('The shape of X train_new index : {}'.format(X_train_new.shape))
# print('The shape of y train_new index : {}'.format(y_train_new.shape))
# print('The shape of X val_new index : {}'.format(X_val_new.shape))
# print('The shape of y val_new index : {}'.format(y_val_new.shape))

# X_new = np.asarray(X_train_new)
# y_new = np.asarray(y_train_new)
# print('y_new labels : {}'.format(np.unique(y_new)))

# # standardisation 
# scl_new = StandardScaler().fit(X = X_new)
# # X_new_std  = scl_new.transform(X = X_new)
...
#         X_train  = scl_new.transform(X = X_train)
#         X_test = scl_new.transform(X = X_test)
...
# averaged_CM_wns = confusion_matrix(save_true, save_predicted)
...
#         color_codes=True,
#         rc=({"font.family": "Dejavu Sans"}))
# plot_confusion_matrix(averaged_CM_wns, classes)
# plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\_averaged_CM_wns_.png"), dpi = 500, bbox_inches="tight")
...
# trans_X_val_wns = scl_new.transform(X = X_val_new)

# # Predict the age of mosquitos preserved in ethanol 

# y_val_pred_wns = best_classifier.predict(trans_X_val_wns)

# accuracy_val_wns = accuracy_score(y_val_new, y_val_pred_wns)
# print("Accuracy: %.2f%%" % (accuracy_val_wns * 100.0))
...
# cm_val_wns = confusion_matrix(y_val_new, y_val_pred_wns)

# plot_confusion_matrix(cm_val_wns, text=True, normalise=True, classes = class_names)
# plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\CM_wns_val.png"), dpi = 500, bbox_inches="tight")


# #%%
# # Summarising precision, f_score, and recall for the validation set
# cr_wn_val = classification_report(y_val_new, y_val_pred_wns)
# print('Classification report : {}'.format(cr_wn_val))

# # save classification report to disk as a csv

# cr_wn_val = pd.read_fwf(io.StringIO(cr_wn_val), header=0)
# cr_wn_val = cr_wn_val.iloc[1:]
# cr_wn_val.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\classification_report_wns_val.csv")
...
# # Using dimensionality reduction (KernelPCA)
...
# ## Set validation procedure
# num_folds = 5 # split training set into 5 parts for validation
# num_rounds = 20 # increase this to 5 or 10 once code is bug-free
# # seed = 4 # pick any integer. This ensures reproducibility of the tests
# scoring = 'accuracy' # score model accuracy
...
# kf_results = pd.DataFrame() # model parameters and global accuracy score
# kf_per_class_results = [] # per class accuracy scores
# kf_coef = pd.DataFrame() # model coeffients
# start = time()


# # define data
# df = pd.read_csv("D:\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\sporozoite_full.csv") 

# X = df.iloc[:,7:] # matrix of features
# y = df["Sporozoite"] # vector of labels

# # Create a pipeline standardisation and dimensionality reduction
...
#         # standardise features and reduce noise by clustering into principle componets 
#         # using Kernel PCA
...
# # plot averaged confusion matrix for training
# averaged_CM_pca = confusion_matrix(save_true, save_predicted)
...
#         color_codes=True,
#         rc=({"font.family": "Dejavu Sans"}))
# plot_confusion_matrix(averaged_CM_pca, classes)
# plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\_average_CM_pca.png"), dpi = 500, bbox_inches="tight")

# #%%
# # Summarising precision, f_score, and recall for the training set
# cr_pca = classification_report(save_true, save_predicted)
# print('Classification report : {}'.format(cr_pca))

# cr_pca = pd.read_fwf(io.StringIO(cr_pca), header=0)
# cr_pca = cr_pca.iloc[1:]
# cr_pca.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\classification_report_pca.csv")


# #%%
# # Results
# kf_results.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_kfCV_record_pca.csv", index=False)
# kf_results = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_kfCV_record_pca.csv")

# # Accuracy distribution
# crf_acc_distrib = kf_results["Accuracy"]
# crf_acc_distrib.columns=["Accuracy"]
# crf_acc_distrib.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_acc_distrib_pca.csv", header=True, index=False)
# crf_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_acc_distrib_pca.csv")
# crf_acc_distrib = np.round(crf_acc_distrib, 2)
# print(crf_acc_distrib)
...
# class_names = np.unique(np.sort(y))
# crf_per_class_acc_distrib = pd.DataFrame(kf_per_class_results, columns=class_names)
# crf_per_class_acc_distrib.dropna().to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_per_class_acc_distrib_pca.csv")
# crf_per_class_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_per_class_acc_distrib_pca.csv", index_col=0)
# crf_per_class_acc_distrib = np.round(crf_per_class_acc_distrib, 2)
# crf_per_class_acc_distrib_describe = crf_per_class_acc_distrib.describe()
# crf_per_class_acc_distrib_describe.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\Results\crf_per_class_acc_distrib_pca.csv")
...
# plt.xlabel(" ")
# plt.ylabel("Prediction accuracy", weight = "bold")
# plt.savefig("D:\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\per_class_acc_distrib_pca.png", dpi = 500, bbox_inches="tight")
