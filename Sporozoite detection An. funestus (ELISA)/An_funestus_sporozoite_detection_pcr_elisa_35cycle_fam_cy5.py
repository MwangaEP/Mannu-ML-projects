...
from imblearn.under_sampling import RandomUnderSampler, NearMiss, CondensedNearestNeighbour, OneSidedSelection
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\Confusion_Matrix_" + figure_name + "_" + ".png"), dpi = 500, bbox_inches="tight")
...
pcr_data_df = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\Phd data\Analysis data\PCR data-35cycles-Cy5-FAM.csv")
pcr_data_df.head()
...
elisa_df = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML analysis\sporozoite_full_edited.csv")
elisa_df = elisa_df.drop(['Unnamed: 0', 'Species', 'Status', 'ID', 'StoTime'], axis = 1)

# # Join the elisa data and PCR data together

infection_data_df = pd.concat([infection_data_df, elisa_df], axis = 0, join = 'outer')
# # infection_data_df.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\Phd data\Analysis data\infection_data_elisa_pcr.csv")
infection_data_df
...
models.append(("LR", LogisticRegressionCV(multi_class = 'ovr', cv = kf, random_state = SEED, max_iter = 3500)))
...
for name, model in models:
    cv_results = cross_val_score(
        model, X_transformed, y_train, cv = kf, scoring = scoring)
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\_algo_selection_.png"), dpi = 500, bbox_inches="tight")
...
with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\classifier.pkl', 'wb') as fid:
     pickle.dump(best_classifier, fid)

# %%

# Results
kf_results.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_kfCV_record.csv", index=False)
kf_results = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_kfCV_record.csv")

# Accuracy distribution
crf_acc_distrib = kf_results["Accuracy"]
crf_acc_distrib.columns=["Accuracy"]
crf_acc_distrib.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_acc_distrib.csv", header=True, index=False)
crf_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_acc_distrib.csv")
...
crf_per_class_acc_distrib.dropna().to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_per_class_acc_distrib.csv")
crf_per_class_acc_distrib = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_per_class_acc_distrib.csv", index_col=0)
...
crf_per_class_acc_distrib_describe.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_per_class_acc_distrib.csv")
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\per_class_acc_distrib_full_wn.png"), dpi = 500, bbox_inches="tight")
...
rskf_results = pd.read_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\crf_kfCV_record.csv")
...
plt.savefig(("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\_feature_impces_full_wn.png"), dpi = 500, bbox_inches="tight")
...
cr_full_wn_val.to_csv("C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\classification_report_full_wn_val.csv")
...
with open('C:\Mannu\Projects\Sporozoite Spectra for An funestus s.s\ML_final_analysis\Results\ELISA_PCR\important_wavenumbers.txt', 'w') as outfile:
