...
df = pd.read_csv("D:\Projects\Isotope Samples\Field data\data\isotope_field.dat", delimiter='\t')
df.head()

#%%
# renaming details in columns

df.loc[df.Cat4 == 'CONTROL', 'Cat4'] = 'Control'
df.loc[df.Cat4 == 'TREATMENT', 'Cat4'] = 'Treatment'
...
df = df.drop(['Cat1', 'Cat2', 'Cat3', 'Cat5', 'StoTime'], axis=1)
print(df.head())
...
X = df.iloc[:,1:] # matrix of features
y = df["Cat4"] # vector of labels
print(X)

seed = 4
rs = ShuffleSplit(n_splits = 10, test_size = 0.25, random_state = seed)
rs.get_n_splits(X)
print(rs)
...
training_set.to_csv("D:\Projects\Isotope Samples\Field data\data\set_training.csv")

#%%
# saving validation set to disk
validation_set = df.iloc[val_index,:]
validation_set.to_csv("D:\Projects\Isotope Samples\Field data\data\set_validation.csv")

#%%
# Import the training set, and define X matrix and a vector of labels
 
df = pd.read_csv("D:\Projects\Isotope Samples\Field data\data\set_training.csv") 
...
for name, model in models:
    cv_results = cross_val_score(
        model, X_new, y, cv=skf, scoring=scoring)
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\selection_algorithm.png", dpi = 500, bbox_inches="tight")
...
# perform a cross validation with stratified k-fold
for train_index, test_index in skf.split(X, y):
    ...
    coef_table = pd.DataFrame(svc_pipe.named_steps['clf'].coef_, columns=X.columns).T
...
for train_index, test_index in skf.split(X, y):
    ...
    probas_ = svc_pipe.fit(X_train, y_train).predict_proba(X_test)
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\cross_v_roc_curve.png", dpi = 500, bbox_inches="tight")
plt.show()

#%%
# Results
skf_results.to_csv("D:\Projects\Isotope Samples\Field data\Output\svc_skfCV_record.csv", index=False)
skf_results = pd.read_csv("D:\Projects\Isotope Samples\Field data\Output\svc_skfCV_record.csv")

# Accuracy distribution
svc_acc_distrib = skf_results["Accuracy"]
svc_acc_distrib.columns=["Accuracy"]
svc_acc_distrib.to_csv("D:\Projects\Isotope Samples\Field data\Output\svc_acc_distrib.csv", header=True, index=False)
svc_acc_distrib = pd.read_csv("D:\Projects\Isotope Samples\Field data\Output\svc_acc_distrib.csv")
...
skf_coef.to_csv("D:\Projects\Isotope Samples\Field data\Output\coef_repeatedCV_.csv")

#%%
coef_1 = pd.read_csv("D:\Projects\Isotope Samples\Field data\Output\coef_repeatedCV_.csv")
print(coef_1)
coef_2 = coef_1.rename(columns = {'Unnamed: 0': 'Wavenumbers'})
coef_2.to_csv("D:\Projects\Isotope Samples\Field data\Output\coef_repeatedCV_1.csv")

#%% plotting coefficients
n_features = 10
skf_coef_df = pd.read_csv("D:\Projects\Isotope Samples\Field data\Output\coef_repeatedCV_1.csv")
skf_coef_df_1 = skf_coef_df.reset_index().set_index('Wavenumbers')
skf_coef_df_1.sort_values(by="coef mean", ascending=False, inplace=True)
coef_plot_data = skf_coef_df_1.drop(["coef sem", "coef mean"], axis=1).T
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\svc_coeffients-2.png", dpi = 500, bbox_inches="tight")

#%%
# class distribution 
class_names = y.sort_values().unique()
svc_per_class_acc_distrib = pd.DataFrame(skf_per_class_results, columns=class_names)
svc_per_class_acc_distrib.dropna().to_csv("D:\Projects\Isotope Samples\Field data\Output\svc_per_class_acc_distrib.csv")
svc_per_class_acc_distrib = pd.read_csv("D:\Projects\Isotope Samples\Field data\Output\svc_per_class_acc_distrib.csv", index_col=0)
svc_per_class_acc_distrib = np.round(svc_per_class_acc_distrib, 2)
svc_per_class_acc_distrib_describe = svc_per_class_acc_distrib.describe()
svc_per_class_acc_distrib_describe.to_csv("D:\Projects\Isotope Samples\Field data\Output\svc_per_class_acc_distrib.csv")

#%%
# plotting class distribution
svc_per_class_acc_distrib = pd.melt(svc_per_class_acc_distrib, var_name="Conc new")
print(svc_per_class_acc_distrib)
sns.set(context="paper",
    style="whitegrid",
    font_scale=2.0,
    rc={"font.family": "Dejavu Sans"})
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\svc_per_class_acc_distrib.png", dpi = 500, bbox_inches="tight")
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\confu_matrix_train.png", dpi = 500, bbox_inches="tight")
...
cr = cr.iloc[1:]
cr.to_csv('D:\Projects\Isotope Samples\Field data\Output\classification_report_train.csv')

# dump/save/serialize the final model to the disk for future prediction. 

with open('D:\Projects\Isotope Samples\Field data\Output\isotope_field_model.pkl', 'wb') as fid:
...
validation= pd.read_csv("D:\Projects\Isotope Samples\Field data\data\set_validation.csv")
...
y_val = validation["Cat4"]

#%%
# Deserializing the final model from the disk to predict new samples
with open('D:\Projects\Isotope Samples\Field data\Output\isotope_field_model.pkl', 'rb') as fid:
    isotope_field_model = pickle.load(fid)

#%%
# Evaluating the final model predicting new samples (validation set)
Y_val_pred = isotope_field_model.predict(X_val)
...
class_names = np.unique(np.sort(y_val))
plot_confusion_matrix(cm, text=True, normalise=True, classes=class_names)
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\conf_matrix_vaidation_balanced.png", dpi = 500, bbox_inches="tight")
...
cr = cr.iloc[1:]
cr.to_csv('D:\Projects\Isotope Samples\Field data\Output\classification_report_validation_balanced.csv')

#%%
# plot number of samples predicted
# Prepare datasets for no. of samples in validation set 

no_val = pd.read_csv('D:\Projects\Isotope Samples\Field data\Output\classification_report_validation_balanced.csv')
print(no_val.head(5))
...
no_train = pd.read_csv('D:\Projects\Isotope Samples\Field data\Output\classification_report_train.csv')
...
plt.savefig("D:\Projects\Isotope Samples\Field data\Output\samples_total used for evaluation.png", dpi = 500, bbox_inches="tight")
