...
#%%
# This program uses standard machine learning to learn parasite signatures 
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig(
                    (
                        r"C:\Mannu\Projects\Mwanga-DBS work\Parasite age\ML analysis\Neg-Pos_results\Confusion_Matrix_" 
                        + figure_name 
                        + "_" 
                        + ".png"
                    ), 
...
base_directory = r"C:\Mannu\Projects\Mwanga-DBS work\Parasite age\ML analysis\Neg-Pos_results"
...
for row in par_df['Cat1']:
    ...
    if row == 'EC':
        Treatment.append('Negative')
    
    elif row == 'LC':
        Treatment.append('Negative')
    
    elif row == 'ET':
        Treatment.append('Positive')
...
print('number of samples per infection status : {}'.format(Counter(train_set['Cat1'])))
...
scl_features = scaler.transform(X = X)
...
for round in range(num_rounds):
    ...
    for train_index, test_index in kf.split(scl_features, y):
