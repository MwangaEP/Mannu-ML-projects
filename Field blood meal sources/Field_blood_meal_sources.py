...
from sklearn.metrics import accuracy_score, confusion_matrix, classification_report, max_error, precision_recall_fscore_support, roc_curve, auc, precision_score, recall_score, f1_score
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig(("C:\Mannu\Projects\Mannu Phd\Confusion_Matrix_" + figure_name + "_" + ".png"), dpi = 500, bbox_inches="tight")
...
non_human_df = pcr_data_df.query('PCR_RESULTS in ["Bovine", "Dog", "Cow_dog"]')
print(Counter(non_human_df["PCR_RESULTS"])) # Checking class distribution
...
non_human_samples = non_human_df['SAMPLE_ID']
non_human_samples_df = blood_field_df.query("ID in @non_human_samples")

# create a new column in positive samples dataframe and name the samples as positives
non_human_samples_df['blood_meal'] = 'Non-human'
non_human_samples_df

# %%

# Concatinating human and bovine bloodfed dataframes together

human_bov_bldfed_df = pd.concat([human_b_samples_df, non_human_samples_df], axis = 0, join = 'outer')
...
X_train, X_test, y_train, y_test = train_test_split(X_res, y_res, test_size= .1, random_state = 42, 
                                                    shuffle = True)
...
SEED  = 42 # np.random.randint(0, 814789)
...
# models.append(("SGD", SGDClassifier(random_state=SEED, loss = 'log_loss', alpha = 1.0, penalty=None, max_iter = 3500)))
...
for name, model in models:
    cv_results = cross_val_score(
        model, X_transformed, y, cv = kf, scoring = scoring)
...
plt.xlabel(" ");
plt.savefig(("C:\Mannu\Projects\Mannu Phd\_algo_selection_.png"), dpi = 500, bbox_inches="tight")
...
for round in range (num_rounds):
    ...
    for train_index, test_index in kf.split(X, y):
        X_train_set, X_val = X[train_index], X[test_index]
        ...
        classifier = LogisticRegression(multi_class = 'ovr', random_state = SEED, max_iter = 3500)
        ...
        n_iter_search = 100
        gridCV = GridSearchCV(verbose = 1,
                        estimator = classifier, param_grid = random_grid, 
        ...
        print("Best: %.2f using %s" % (gridCV_result.best_score_,
                                        gridCV_result.best_params_))

        # Insert the best parameters identified by randomized grid search into the base classifier
        best_classifier = classifier.set_params(**gridCV_result.best_params_, n_jobs = -1)
...
with open('C:\Mannu\Projects\Mannu Phd\classifier.pkl', 'wb') as fid:
     pickle.dump(best_classifier, fid)

# %%

# Results
kf_results.to_csv("C:\Mannu\Projects\Mannu Phd\crf_kfCV_record.csv", index=False)
kf_results = pd.read_csv("C:\Mannu\Projects\Mannu Phd\crf_kfCV_record.csv")

# Accuracy distribution
crf_acc_distrib = kf_results["Accuracy"]
crf_acc_distrib.columns=["Accuracy"]
crf_acc_distrib.to_csv("C:\Mannu\Projects\Mannu Phd\crf_acc_distrib.csv", header=True, index=False)
crf_acc_distrib = pd.read_csv("C:\Mannu\Projects\Mannu Phd\crf_acc_distrib.csv")
...
crf_per_class_acc_distrib.dropna().to_csv("C:\Mannu\Projects\Mannu Phd\crf_per_class_acc_distrib.csv")
crf_per_class_acc_distrib = pd.read_csv("C:\Mannu\Projects\Mannu Phd\crf_per_class_acc_distrib.csv", index_col=0)
...
crf_per_class_acc_distrib_describe.to_csv("C:\Mannu\Projects\Mannu Phd\crf_per_class_acc_distrib.csv")
...
plt.savefig(("C:\Mannu\Projects\Mannu Phd\per_class_acc_distrib_full_wn.png"), dpi = 500, bbox_inches="tight")
...
visualize(figure_name, classes, y_val_pred, y_test)

#%%
# Summarising precision, f_score, and recall for the validation set
cr_full_wn_val = classification_report(y_test, y_val_pred)
...
cr_full_wn_val.to_csv("C:\Mannu\Projects\Mannu Phd\classification_report_full_wn_val.csv")
...
def my_logistic_report(X_test_,y_test_, threshold=0.5): # you could make it even more general!
    ...
    precision = precision_score(y_test_, y_hat, pos_label = 'Human')
    recall = recall_score(y_test_, y_hat, pos_label = 'Human')
    f1score = f1_score(y_test_, y_hat, pos_label = 'Human')
    cm_labeld = pd.DataFrame(cm, index=['Actual :Human','Actual : Non-human'], columns=['Predict : Human','Predict :Non-human'])
    ...
    print("-----------------------------------------")
    return cm
cm = my_logistic_report(trans_X_val, y_test)
# cm_labeld_ = my_logistic_report(trans_X_val, y_test)
# cm_labeld_.to_csv("C:\Mannu\Projects\Mannu Phd\cm_labeld_.csv")
